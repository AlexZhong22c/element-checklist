const path = {
  ...
  mutations: {
    ...
    SET_CUSTOM_PWDS (state, custom_pwds) {
      ...
      localStorage.setItem('element-checklist-custom_pwd_list', JSON.stringify(custom_pwds));
    },
    ADD_CUSTOM_PWDS (state, custom_pwd) {
      state.custom_pwds.push(custom_pwd);
      localStorage.setItem('element-checklist-custom_pwd_list', JSON.stringify(state.custom_pwds));
    },
